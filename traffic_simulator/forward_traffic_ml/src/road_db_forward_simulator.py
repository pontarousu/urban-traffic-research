...
def spawn_vehicles(
    *,
    ...
    for _ in range(max(0, spawn_count)):
        source = choose_weighted(source_candidates, rng)
        if not source:
            break
        ...
        spawn_stats_by_category[category] += 1
        spawn_stats_by_edge[edge_id] += 1
        next_vehicle_id += 1
        spawned += 1
...
def record_crossings(
    *,
    ...
    for observation in observations_by_edge.get(edge_id, []):
        ...
        if previous_position <= target < next_position:
            counts[(bin_min, observation.observation_id)] += 1
    ...
    *,
    ...
    for elapsed_sec in range(0, total_seconds, time_step_sec):
        ...
        if scheduled_spawn > 0:
            ...
            next_vehicle_id, spawned = spawn_vehicles(
                active=active,
    ...
    return {
        ...
        "summary": {
            ...
            "active_vehicle_avg": sum(active_samples) / len(active_samples) if active_samples else 0.0,
            "active_vehicle_max": max(active_samples) if active_samples else 0,
