...
# =====================================================================
def main():
    # osmnxキャッシュフォルダを明示設定（オフライン対応）
    ox.settings.cache_folder = str(Path(__file__).resolve().parents[1] / ".osmnx_cache")
    ...
    print(f"  候補ノードプール: {len(local_ids)} ノード")

    # KD-Treeを構築し、ユークリッド距離 ≤ MAX_EUCLIDEAN_GAP の全ペアを列挙
    tree = cKDTree(local_xy)
    close_pairs = tree.query_pairs(r=MAX_EUCLIDEAN_GAP)  # frozenset of (i, j) tuples
    print(f"  ユークリッド距離 ≤ {MAX_EUCLIDEAN_GAP}m のペア数: {len(close_pairs)}")

    # 下限距離（50m未満）を除外し、条件②（迂回比）でフィルタリング
    candidates = []
    G_undirected = G_proj.to_undirected()  # 無向グラフ変換を1回だけ実行
    checked = 0
    for (ii, jj) in close_pairs:
        xA, yA = local_xy[ii]
        xB, yB = local_xy[jj]
        ...
        # ネットワーク距離計算（条件②: 迂回比 ≥ MIN_NETWORK_RATIO）
        try:
            nd = nx.shortest_path_length(G_undirected, nA, nB, weight='length')
        ...
        checked += 1
        if checked % 500 == 0:
            print(f"    進捗: {checked}/{len(close_pairs)} ペアを検査済み, 候補: {len(candidates)}")

    search_elapsed = time.time() - search_start
    print(f"  全数探索完了: {len(candidates)} 候補を発見 ({search_elapsed:.1f}秒)")
    if len(candidates) == 0:
        print("  No candidates found. Try relaxing MAX_EUCLIDEAN_GAP or MIN_NETWORK_RATIO.")
        return

    # ------------------------------------------------------------------
    # 5b. 2段階フィルタリング: プロキシスコアで上位候補を選抜
    # ------------------------------------------------------------------
    # ROI評価（IoU再計算）は1候補あたり数十秒かかるため、
    # 全候補を評価するのは計算量的に非現実的。
    # そこで「迂回比 / ユークリッド距離」のプロキシスコア（推定ROI）で
    # 事前にソートし、上位のみをROI評価対象にする。
    #
    # プロキシスコアの直感:
    #   - 迂回比が大きい = 新道の効果が大きい（分子に相当）
    #   - ユークリッド距離が短い = 建設コストが小さい（分母に相当）
    #   → この比率が高い候補ほど、真のROIも高い可能性が高い
    ROI_EVAL_LIMIT = 100  # ROI評価する上位候補数

    def proxy_score(c):
        """迂回比/ユークリッド距離のプロキシスコア（ROI推定値）"""
        _, _, eucl, nd, _, _, _, _ = c
        if nd == float('inf'):
            return float('inf')  # 完全分断ペアは最優先
        return (nd / eucl) / eucl

    candidates.sort(key=proxy_score, reverse=True)
    eval_candidates = candidates[:ROI_EVAL_LIMIT]
    print(f"  プロキシスコア上位 {len(eval_candidates)} 候補をROI評価対象に選抜")

    # ------------------------------------------------------------------
    # 6. Evaluate ROI for each candidate edge
    # ------------------------------------------------------------------
    print(f"6. Evaluating ROI for {len(eval_candidates)} candidates (recomputing IoU with virtual edge)...")
    roi_results = []

    # 選抜候補に対してROI評価を実行
    for idx, (nA, nB, eucl, nd, xA, yA, xB, yB) in enumerate(eval_candidates):
        ...
        print(f"  [{idx+1}/{len(eval_candidates)}] edge={eucl:.0f}m, ΔIoU_sum={delta_iou_sum:.4f}, ROI={roi:.6f}")
